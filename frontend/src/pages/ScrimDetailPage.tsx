...
import type { Estadistica, Feedback, ParticipanteScrim, Reporte, Scrim } from '../api/types'
...
export function ScrimDetailPage() {
  ...
  const titulares = useMemo<ParticipanteScrim[]>(
    () => scrim?.equipos.flatMap((equipo) => equipo.jugadores) ?? [],
    [scrim],
  )

  const confirmacionPorUsuario = useMemo(
    () => new Map(titulares.map((participante) => [participante.username, participante])),
    [titulares],
  )
  ...
  const scrimJuego = scrim?.juego

  useEffect(() => {
    if (!scrimJuego) return
    const defaultRol = getRolesForJuego(scrimJuego)[0]?.value ?? 'DUELIST'
    setRol(defaultRol)
  ...
  const handleConfirmarPendientes = async () => {
    if (!id || !isMod) return
    const pendientes = titulares.filter((participante) => !participante.confirmado)
    if (pendientes.length === 0) return

    setActionMsg('')
    setError('')
    try {
      let updated: Scrim | null = null
      for (const participante of pendientes) {
        updated = await scrimsApi.confirmar(id, participante.username)
      }
      if (updated) setScrim(updated)
      setActionMsg(`Confirmaciones registradas: ${pendientes.length}`)
    } catch (err) {
      setError(err instanceof Error ? err.message : 'Error al confirmar titulares')
    }
  }

  const handleIniciar = () => {
    if (!id || !scrim) return
    if (scrim.estado !== 'CONFIRMADO') {
      const pendientes = Math.max(titulares.length - titulares.filter((p) => p.confirmado).length, 0)
      setActionMsg('')
      setError(
        pendientes > 0
          ? `Faltan ${pendientes} confirmación${pendientes === 1 ? '' : 'es'} para iniciar.`
          : 'El scrim todavía no está confirmado.',
      )
      return
    }
    runAction(() => scrimsApi.iniciar(id), 'Scrim iniciado')
  }
  ...
  const communityPlayers = useMemo(() => {
    ...
    titulares.forEach((participante) => {
      byUsername.set(participante.username, participante.username)
    })
    aceptados.forEach((postulacion) => {
      byUsername.set(postulacion.username, postulacion.username)
    })
    byUsername.delete(user?.username ?? '')
    return [...byUsername.values()].sort((a, b) => a.localeCompare(b))
  }, [aceptados, titulares, user?.username])
  ...
  const confirmados = titulares.filter((participante) => participante.confirmado).length
  const totalConfirmaciones = titulares.length || aceptados.length
  const confirmacionesPendientes = Math.max(totalConfirmaciones - confirmados, 0)
  const miConfirmacion = user ? confirmacionPorUsuario.get(user.username) : undefined
  const miAsistenciaConfirmada = miConfirmacion?.confirmado ?? false
  const puedeConfirmarAsistencia =
    scrim.estado === 'LOBBY_ARMADO'
    && miPostulacion?.estado === 'ACEPTADA'
    && !miAsistenciaConfirmada
  const puedeIniciar = scrim.estado === 'CONFIRMADO'
  const puedeConfirmarPendientes =
    isMod && scrim.estado === 'LOBBY_ARMADO' && confirmacionesPendientes > 0
    ...
    >
        ...
        <div className="scrim-detail-hero-body">
          ...
          <div className="scrim-detail-pills">
            ...
            {totalConfirmaciones > 0 && (
              <span className={`scrim-pill${confirmacionesPendientes === 0 ? ' scrim-pill-ready' : ''}`}>
                {confirmados}/{totalConfirmaciones} confirmados
              </span>
            )}
          </div>
        ...
        <section className="detail-panel">
          ...
          <div className="detail-panel-body">
              ...
              <div className="detail-status-card">
                ...
                {miPostulacion.estado === 'ACEPTADA' && (
                  puedeConfirmarAsistencia ? (
                    ...
                    <span className="detail-confirmation-chip">
                      {miAsistenciaConfirmada ? 'Asistencia confirmada' : 'Esperando lobby armado'}
                    </span>
                  )
                )}
              </div>
            ...
            <div className="detail-action-bar">
              {puedeConfirmarPendientes && (
                <button
                  type="button"
                  className="btn btn-primary btn-sm"
                  onClick={handleConfirmarPendientes}
                >
                  Confirmar titulares pendientes
                </button>
              )}
              <button
                type="button"
                ...
                title={
                  puedeIniciar
                    ? 'Iniciar scrim'
                    : `Faltan ${confirmacionesPendientes} confirmación${confirmacionesPendientes === 1 ? '' : 'es'}`
                }
              >
                Iniciar
              </button>
      ...
      {puedeGestionarLobby && (
        <section className="detail-panel lobby-panel">
          <header className="detail-panel-header">
            ...
            {totalConfirmaciones > 0 && (
              <span
                className={`detail-count-badge${confirmacionesPendientes === 0 ? ' detail-count-badge-ready' : ''}`}
              >
                {confirmados}/{totalConfirmaciones} confirmados
              </span>
            )}
                  ...
                  >
                    <div className="lobby-roster-main">
                      ...
                      <div className="lobby-roster-info">
                        ...
                        <span
                          className={`lobby-confirmation-status${confirmacionPorUsuario.get(p.username)?.confirmado ? ' confirmed' : ''}`}
                        >
                          {confirmacionPorUsuario.get(p.username)?.confirmado
                            ? 'Confirmado'
      ...
      <div className="detail-community-grid">
        <section className="detail-panel">
          ...
          {user && communityPlayers.length > 0 && (
